...
const productosModelo = require('./models/productos.models.js')
...
prodRouter.get('/:pid',async (req,res)=>{
    try{ 
        ...
        if(!isNaN(id)){
            ...
            let product = await productosModelo.findOne({id});
            if(product){
...
prodRouter.get('/',async(req,res)=>{
    try{ 
        let product = await productosModelo.find({});
        let nQuery = []
    ...
    try{ 
       const {title,description,code,price,status,stock,category,thumbnails, ...propiedadNoAceptada}= req.body;
        if(Object.keys(propiedadNoAceptada).length > 0){
            ...
            let existe = await productosModelo.findOne({code})
            if(existe){
            ...
            // si la promesa falla
           res.status(500).json({ error: 'Hubo un error al agregar los productos.' });
        }
})
